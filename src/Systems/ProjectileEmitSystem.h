	...
	public:
			...
			if (event.symbol == SDLK_SPACE) {
				for (auto entity: GetSystemEntities()) {
					if (entity.HasComponent<CameraFollowComponent>()) {
						...
						// If parent entity direction is controlled by the keyboard keys, modify the direction of the
						// projectile accordingly
						...
						projectile.AddComponent<ProjectileComponent>(projectileEmitter.isFriendly,
							projectileEmitter.hitPercentDamage, projectileEmitter.projectileDuration);
					}
				}
			}
		}
		...
		void Update(std::unique_ptr<Registry>& registry) {
			for (auto entity: GetSystemEntities()) {
				...
				else if (SDL_GetTicks() - projectileEmitter.lastEmissionTime > 
						static_cast<Uint32>(projectileEmitter.repeatFrequency))
				{
					glm::vec2 projectilePosition = transform.position;
					if (entity.HasComponent<SpriteComponent>()) {
					...
					projectile.AddComponent<TransformComponent>(
							projectilePosition, glm::vec2(1.0, 1.0), 0.0);
